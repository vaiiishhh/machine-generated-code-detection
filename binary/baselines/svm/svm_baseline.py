...
Original file is located at
    https://colab.research.google.com/drive/1Mzioxfm_hJWRIxf8yysNx-tpZXSJNego
"""
...
def compute_and_save_learning_curve(pipeline, X_train, y_train, lang_dir, language, n_jobs=-1):
    ...
    ax.set_title(f"Learning Curve - SVM (Nyström RBF) - {language}")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()

    png_path = os.path.join(lang_dir, "learning_curve.png")
    ...
    json_path = os.path.join(lang_dir, "learning_curve.json")
...
if __name__ == "__main__":
    ...
    try:
        ...
        SAVE_DIR = "/content/drive/MyDrive/codet_m4_svm"
    except Exception:
        SAVE_DIR = "./codet_m4_svm"
    ...
    for LANGUAGE in LANGUAGES_TO_RUN:
        ...
        def _precomp(df):
            return df[feat_col].tolist() if feat_col else [{}] * len(df)
        def _langs(df):
            return df["_lang"].tolist()
        ...
        feat_names_path = os.path.join(lang_dir, "feature_names.json")
        ...
        model_path = os.path.join(lang_dir, f"svm_{LANGUAGE.replace('+','p')}.joblib")
        ...
        if not (len(srcs) == 1 and "unknown" in srcs):
            ...
            src_csv_path = os.path.join(lang_dir, "per_source_metrics.csv")
            pd.DataFrame(rows).to_csv(src_csv_path, index=False)
    ...
    summary_path = os.path.join(SAVE_DIR, "summary_all_languages.json")
    with open(summary_path, "w") as fh:
        json.dump(summary, fh, indent=2)

    global_src_csv_path = os.path.join(SAVE_DIR, "per_source_all_languages.csv")
    pd.DataFrame(global_src_rows).to_csv(global_src_csv_path, index=False)
    ...
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("Score")
    ax.set_title("SVM (Nyström RBF) - Per-Language Results")
    ax.legend()
    ax.grid(True, axis="y", alpha=0.3)
    plt.tight_layout()

    summary_plot_path = os.path.join(SAVE_DIR, "summary_all_languages.png")
